...
public final class AFFKeyboard {
    ...
    private static int getDecorViewInvisibleHeight(final Context context, @NonNull final Window window) {
     ...
     * @param listener The soft input changed listener.
     */
    public static void registerSoftInputChangedListener(final Context context,
        ...
        final int[] decorViewInvisibleHeightPre = {getDecorViewInvisibleHeight(context, window)};
        contentView.getViewTreeObserver()
                .addOnGlobalLayoutListener(new OnGlobalLayoutListener() {
                    @Override
                    public void onGlobalLayout() {
                        int height = getDecorViewInvisibleHeight(context, window);
     ...
     * @param window The window.
     */
    public static void fixAndroidBug5497(final Context context, @NonNull final Window window) {
//        int softInputMode = window.getAttributes().softInputMode;
//        window.setSoftInputMode(softInputMode & ~WindowManager.LayoutParams.SOFT_INPUT_ADJUST_RESIZE);
        final FrameLayout contentView = window.findViewById(android.R.id.content);
        final View contentViewChild = contentView.getChildAt(0);
        final int paddingBottom = contentViewChild.getPaddingBottom();
        final int[] contentViewInvisibleHeightPre5497 = {getContentViewInvisibleHeight(context, window)};
        contentView.getViewTreeObserver()
                .addOnGlobalLayoutListener(new OnGlobalLayoutListener() {
                    @Override
                    public void onGlobalLayout() {
                        int height = getContentViewInvisibleHeight(context, window);
                        if (contentViewInvisibleHeightPre5497[0] != height) {
                            contentViewChild.setPadding(
                                    contentViewChild.getPaddingLeft(),
                                    contentViewChild.getPaddingTop(),
                                    contentViewChild.getPaddingRight(),
                                    paddingBottom + getDecorViewInvisibleHeight(context, window)
                            );
    ...
    private static int getContentViewInvisibleHeight(final Context context, final Window window) {
    ...
    private static int getNavBarHeight(final Context context) {
